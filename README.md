...
界面行为，是视图所有交互的实际表现。在视图中，根据使用对象，将界面行为分为视图行为和部件行为。
...
视图行为的使用目标是开放的，它既可以直接作用于视图内容，也可以跟视图无关。
...
移动端界面数据信息内容，相对于 pc 端而言偏弱；部件行为相对 pc 端而言，则要强一些。从模型构成来说，我们应当为移动端的部件行为补充更多有意义的模型内容，将其集成到 iBiz 前端模型中。